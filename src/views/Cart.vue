<template>
  <div>
    ...
    <nav-bread>
      <router-link slot="secBread" to="/">购物车</router-link>
    </nav-bread>
    <!-- 垃圾桶 -->
    <svg
    ...
    <modal :mdShow="nullCar" v-on:close="closeModal">
      <p slot="message">
        <span>空购物车</span>
      </p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="nullCar=false">关闭</a>
      </div>
    </modal>
...
export default {
  ...
  computed: {
    //一个计算属性所用到的数据发生了改变，则计算属性也会立即改变
    checkAllFlag() {
      // 控制全选
      return this.cartList.length == this.checkedCount; // 勾选的商品种数=购物车商品列表的商品种数时，返回true代表全选。
    },
  ...
  methods: {
    ...
    handlePay() {
      ...
      axios.get("/users/cart/pay").then(response => {
